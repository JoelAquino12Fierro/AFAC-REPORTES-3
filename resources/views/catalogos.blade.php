    ...
    <body>
        <div class="px-4 py-10 md:px-10 md:py-14 flex flex-col md:flex-row gap-4 rounded-md">
            <div class="basis-1/3 mr-2 bg-white rounded-md p-4">
                ...
                <a href="#" onclick="showForm('create-module-form')"
                    ...
                    <span class="ms-3">AGREGAR MÓDULOS</span>
                </a>
            </div>
            ...
            <div class="flex-1 bg-white rounded-md p-6" id="area-form">
                <form action="{{ route('register.area') }}" method="POST">
                    ...
                    <button type="submit" class=" mt-4 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Registrar</button>
                </form>
            </div>
            ...
            <div class="flex-1 bg-white rounded-md p-6 hidden" id="module-form">
                <form action="{{ route('register.module') }}" method="POST">
                    ...
                    <select id="system" name="system" onchange="updateModules()"
                        ...
                        <option value="SistemaCitasAFAC">SISTEMA CITAS AFAC</option>
                        <option value="SistemaCitasSICT">SISTEMA CITAS SICT</option>
                    </select>
            ...
            <div class="flex-1 bg-white rounded-md p-6 hidden" id="system-form">
                <form action="{{ route('register.system') }}" method="POST">
                    @csrf
                    <label for="system" class="block mb-2 text-sm font-medium text-gray-900">NOMBRE DEL SISTEMA</label>
            ...
            <div class="flex-1 bg-white rounded-md p-6 hidden" id="create-module-form">
                <form action="{{ route('register.newModule') }}" method="POST">
                    @csrf
                    <label for="newModule" class="block mb-2 text-sm font-medium text-gray-900">AGREGAR NUEVO MODULO</label>
                    <input type="text" id="newModule" name="newModule"
                        class="uppercase bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
                        placeholder="Ingresa aquí..." required />
                    <button type="submit" class="mt-4 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Registrar</button>
                </form>
        ...
        <script>
            ...
            function updateModules() {
                const modules = {
                    'SistemaCitasAFAC': ['Administrador de Accesos', 'Administrador de Sedes', 'Catalogos', 'Administrador de Horarios', 'Administrador de Citas'],
                    'SistemaCitasSICT': ['Administrador de Accesos', 'Administrador de Sedes', 'Catalogos', 'Administrador de Horarios', 'Administrador de Citas']
                };
